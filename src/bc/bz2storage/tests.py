...
To configure a bz2storage, import bc.bz2storage and use the
bz2storage tag:
...
Now let's open the database compressed:
    ...
    >>> db = ZODB.DB(bc.bz2storage.Bz2Storage(
...
Make sure the wrapping methods do what's expected.
    ...
    >>> s = bc.bz2storage.Bz2Storage(ZODB.MappingStorage.MappingStorage())
...
def dont_double_compress():
    ...
    >>> data = b'.z'+b'x'*80
    >>> store = bc.bz2storage.Bz2Storage(ZODB.MappingStorage.MappingStorage())
...
class FileStorageBz2Tests(ZODB.tests.testFileStorage.FileStorageTests):
    ...
    def open(self, **kwargs):
        self._storage = bc.bz2storage.Bz2Storage(
...
class FileStorageBz2TestsWithBlobsEnabled(
    ...
    def open(self, **kwargs):
        ...
        self._storage = bc.bz2storage.Bz2Storage(self._storage)
...
class FileStorageBz2RecoveryTest(
    ...
    def setUp(self):
        ...
        self._dst = bc.bz2storage.Bz2Storage(
...
class FileStorageClientBz2ZEOBz2Tests(FileStorageZEOBz2Tests):
    ...
    def _wrap_client(self, client):
        return bc.bz2storage.Bz2Storage(client)
...
class TestIterator(unittest.TestCase):
    ...
    def test_iterator_closes_underlying_explicitly(self):
        # https://github.com/zopefoundation/bc.bz2storage/issues/4
        ...
        storage = Storage()
        zstorage = bc.bz2storage.Bz2Storage(storage)
...
class TestServerBz2Storage(unittest.TestCase):
    ...
    def test_load_doesnt_decompress(self):
        ...
        # See https://github.com/zopefoundation/bc.bz2storage/issues/5
        map_store = ZODB.MappingStorage.MappingStorage()
        store = bc.bz2storage.Bz2Storage(map_store)
        ...
        server_store = bc.bz2storage.ServerBz2Storage(map_store)
...
def test_suite():
    ...
    class Bz2HackLayer:
        ...
        orig = [bc.bz2storage.compress] # []s hide the function :)

        @classmethod
        def setUp(self):
            bc.bz2storage.transform = (
                lambda data: data and (b'.z'+bz2.compress(data)) or data
                )

        @classmethod
        def tearDown(self):
            [bc.bz2storage.compress] = self.orig
