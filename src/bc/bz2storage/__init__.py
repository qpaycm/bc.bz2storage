...
def compress(data):
    if data and (len(data) > 20) and data[:2] != b'.z':
        compressed = b'.z'+bz2.compress(data)
        if len(compressed) < len(data):
            return compressed
    return data

def decompress(data):
    return data[:2] == b'.z' and bz2.decompress(data[2:]) or data

class ServerBz2Storage(Bz2Storage):
    """Use on ZEO storage server when Bz2Storage is used on client

    Don't do conversion as part of load/store, but provide
    pickle decoding.
    """

    copied_methods = Bz2Storage.copied_methods + (
...
class _Iterator(object):
    ...
    # (https://github.com/qpaycm/bc.bz2storage/issues)
